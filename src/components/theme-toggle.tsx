    ...
    <DropdownMenu.Root>
      <DropdownMenu.Trigger asChild>
        <button
          ...
          className={cn(
            "focus-ring inline-flex h-9 w-9 items-center justify-center rounded-md text-slate-600 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-800",
            className,
          )}
        >
      ...
      <DropdownMenu.Portal>
        <DropdownMenu.Content
          align="end"
          sideOffset={6}
          className="z-50 w-56 rounded-md border border-slate-200 bg-white p-2 shadow-md dark:border-slate-800 dark:bg-slate-900"
        >
          <DropdownMenu.Label className="px-2 pb-1 pt-1 text-xs font-medium uppercase tracking-wide text-slate-400 dark:text-slate-500">
            Appearance
          </DropdownMenu.Label>
          ...
          <div className="grid grid-cols-6 gap-1 px-1 py-1">
            {ACCENTS.map((a) => (
              <button
                ...
                title={a.label}
                className={cn(
                  "focus-ring flex h-7 w-7 items-center justify-center rounded-full ring-offset-2 ring-offset-white dark:ring-offset-slate-900",
                  accent === a.id && "ring-2 ring-slate-400 dark:ring-slate-500",
                )}
              >
