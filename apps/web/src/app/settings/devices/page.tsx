...
function DevicesContent() {
  ...
  const revokeOthers = useMutation({
    ...
    onSuccess: () => {
      ...
      useToastStore.getState().push("success", "Signed out of all other devices");
    },
  });
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-semibold text-slate-900 dark:text-slate-100">Devices &amp; sessions</h1>
        {active.length > 1 && (
      ...
      <Card>
        <CardHeader title="Active sessions" description="Revoke access for any device you don't recognize." />
            ...
            <li key={s.id} className="flex items-center justify-between gap-4 px-5 py-3">
              <div>
                <div className="flex items-center gap-2 text-sm font-medium text-slate-900 dark:text-slate-100">
                  {s.clientType === "CLI" ? "CLI" : "Browser"}
                  {s.deviceName && <span className="text-slate-500 dark:text-slate-400">· {s.deviceName}</span>}
                  {s.current && (
